        ...
        internal void AnimateColor(Color c, int seconds = 0)
        {
            ...
            foreach (var path in strips.Values)
            {
                ...
                else
                {
                    ...
                    var animation = new ColorAnimation() { To = c, Duration = new Duration(TimeSpan.FromSeconds(seconds)) };
        ...
        internal void AnimateGradient(Color[] colors, int seconds)
        {
            if (colors.Length > 1)
            {
                ...
                foreach (var path in strips.Values)
                {
                    ...
                    if (brush == null || brush.GradientStops.Count != colors.Length)
                    {
        ...
        internal void AnimateStrip(int index, Color color, int seconds)
        {
            ...
            if (this.strips.TryGetValue(index, out Path path))
            {
                ...
                else
                {
                    ...
                    var animation = new ColorAnimation() { To = color, Duration = new Duration(TimeSpan.FromSeconds(seconds)) };
                    Storyboard story = new Storyboard();
                    Storyboard.SetTarget(animation, brush);
                    Storyboard.SetTargetProperty(animation, "Color");
                    story.Children.Add(animation);
        ...
        internal void AnimateStripGradient(int index, Color[] colors, int seconds)
        {
            if (this.strips.TryGetValue(index, out Path path))
            {
                ...
                else
                {
                    ...
                    if (existing == null || existing.GradientStops.Count != colors.Length)
                    {
                        path.Stroke = brush;
                        existing = brush;
                    }
                    ...
                    {
                        var animation = new ColorAnimation() { To = g.Color, Duration = new Duration(TimeSpan.FromSeconds(seconds)) };
                        Storyboard.SetTargetProperty(animation, "Color");
                        Storyboard.SetTarget(animation, existing);
                        sb.Children.Add(animation);
                    }
