...
class LightingDesigner:
    ...
    def __init__(self, server, msgbus, sensei, internet_address, config):
        ...
        self.state_machine = StateMachine(
            ...
            self.config.custom_animation_timeout,
            self.config.reboot_timeout,
        )
    ...
    def _set_power_state(self, option):
        if option == "on":
            ...
            # remain until the configured custom_timeout.
            self.state_machine.set_state(States.CUSTOM)
        elif option == "reboot":
            log.info("### rebooting...")
            self.state_machine.set_state(States.REBOOT)
            self.color_override = False
            self.animations = None
            self.reboot = True
            self._fade_to_black()
    ...
    def process_next_message(self):
        ...
        if parts[0] == "ping":
            ...
            if len(parts) > 4:
                ...
                if len(colors) > 0:
                    log.info(
                        "### gradient {} {} colors: {}".format(target, strip, colors)
                    )
        ...
        while self.running:
            ...
            if bridge and (
                ...
                if not self.bridge_initialized and bridge.lights_on is not None:
                    ...
                    self.state_machine.set_state(self.power_state)
            ...
            if self.power_state == States.CUSTOM and self.reboot:
