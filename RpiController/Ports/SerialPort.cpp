...
public:
	...
	void flush()
	{
		PurgeComm(this->handle, PURGE_TXABORT | PURGE_RXABORT | PURGE_TXCLEAR | PURGE_RXCLEAR);
	}
...
public:
	...
	int setAttributes(int baudRate, Parity parity, int dataBits, StopBits stopBits, Handshake handshake, int readTimeout, int writeTimeout)
	{
		...
		tty.c_oflag &= ~OPOST;
		tty.c_cc[VMIN] = 0;             // read never blocks forever
		if (readTimeout != -1)
		{
